...
class DataValidation:
    ...
    def validate_columns(self,dataframe:pd.DataFrame)->bool:
        try:
            ...
            logging.info(f"Number_of_columns is: {number_of_columns}")
            logging.info(f"Number of columns is:{len(dataframe.columns)}")
    ...
    def detect_dataset_drift(self,base_df,current_df,threshold_value=0.05)->bool:
        try:
            status=True
            report={}
            for column in base_df.columns:
                ...
                if threshold_value<=is_same_dist.pvalue:
                    is_found=False
                else:
                    is_found=True
                    status=False
